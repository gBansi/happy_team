    ...
    return (
        <div className=' items-center rounded-xl xl:h-[600px] h-fit overflow-hidden mx-auto my-4 text-white py-11 bg-[#332645] px-5'>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3  gap-4 mx-auto mt-10">
                {[...Array(3)].map((_, index) => (
                    <div key={index} className="flex items-center justify-center overflow-hidden h-[300px] xl:w-[400px] w-fit mx-auto rounded-lg">
                        {/* Ensure the index is within the array bounds */}
                        <img src={images[(currentImageIndex + index) % images.length].imgSrc} alt={images[(currentImageIndex + index) % images.length].alt} className="object-cover w-full h-auto rounded-lg" />
                    </div>
                ))}
            </div>
        </div>
